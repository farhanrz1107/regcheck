...
# FIX: We use a radio button instead of tabs so Python knows exactly what mode you are in.
calibration_mode = st.sidebar.radio(
    "Choose your method:",
    ("Visual Reference (Draw Line)", "Standard Scale Factor")
)
...
if uploaded_file is not None:
    try:
        ...
        # FIX: The Invisible Cloud Bug Workaround
        # We save the image to the cloud server's local storage briefly, then reopen it.
        # This guarantees Streamlit's web server can "see" and serve the file to the canvas.
        temp_path = "temp_background.png"
        pil_image.save(temp_path, format="PNG")
        safe_pil_image = Image.open(temp_path)
        
        # Resize image slightly for better web performance
        max_width = 800
        w, h = safe_pil_image.size
        if w > max_width:
            ratio = max_width / float(w)
            new_h = int(float(h) * float(ratio))
            safe_pil_image = safe_pil_image.resize((max_width, new_h), Image.Resampling.LANCZOS)
            w, h = safe_pil_image.size
        ...
        with col1:
            ...
            # FIX: Logic to disable drawing when Standard Scale is selected
            if calibration_mode == "Visual Reference (Draw Line)":
                st.caption("🖱️ Draw a line over your reference object.")
                current_drawing_mode = "line"
            else:
                st.caption("👁️ View-only mode. (Switch to 'Visual Reference' to draw).")
                current_drawing_mode = "transform" # Turns off the drawing tool
            ...
            if canvas_result.json_data is not None and calibration_mode == "Visual Reference (Draw Line)":
